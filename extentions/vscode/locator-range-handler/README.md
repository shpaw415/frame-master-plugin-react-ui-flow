# Frame Master React UI Flow Locator Range Handler

This VS Code extension handles LocatorJS links generated by the `frame-master-react-ui-flow` plugin. It opens the target source file and expands the selection to the matching JSX element or fragment instead of only moving the cursor to a single point.

## Features

- Handles `vscode://.../open` locator URIs.
- Handles `vscode://.../preview` URIs for direct preview-panel opening from shell scripts or other system calls.
- Handles `vscode://.../webview` locator URIs by opening a VS Code webview panel.
- Can load a configured local preview URL inside an iframe and relay LocatorJS clicks from that embedded app back into the editor.
- Opens local files for Linux and Windows environments.
- Opens `vscode-remote://wsl+...` files for WSL targets.
- Expands the editor selection to the full JSX node at the locator position.

## Preview Mode

When the plugin is configured with `useWebview: true` and a `webviewUrl`, the extension opens a reusable preview panel beside the editor.

- The panel loads the configured app URL in an iframe.
- The plugin appends `frameMasterPreview=vscode` to that URL.
- Inside preview mode, the plugin patches LocatorJS VS Code links so they post messages to the parent panel instead of trying to navigate the iframe.
- The extension receives those messages and opens the matching source range in VS Code.

On VS Code startup, the extension also scans workspace `frame-master.config.ts|mts|js|mjs` files. If it finds `UIFlowPlugin({ editor: "vscode", useWebview: true, webviewUrl: ... })`, it polls that preview URL and auto-opens the panel when the dev server becomes reachable, even before you click a Locator target.

You can also open the preview directly from the command line without passing a fake source location:

```sh
code --open-url "vscode://m2tech-solutions.frame-master-react-ui-flow-locator-range-handler/preview?previewUrl=http%3A%2F%2Flocalhost%3A3001&previewTitle=Certorify%20Preview"
```

## Local Packaging

Build a locally installable VSIX with:

```sh
npm run package:vsix
```

This writes `frame-master-react-ui-flow-locator-range-handler.vsix` in the extension folder.

Install it into VS Code with:

```sh
code --install-extension frame-master-react-ui-flow-locator-range-handler.vsix --force
```
