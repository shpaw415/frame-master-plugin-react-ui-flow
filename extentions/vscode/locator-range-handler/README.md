...
- Watches for the plugin's preview marker file opening and converts that fake file-open into a real preview panel open in the current VS Code instance.
...
The plugin can also open the preview directly in the current VS Code instance without `--open-url`. On dev-server start it writes a JSON marker file and opens that file with `code -r`. This extension detects the marker document, opens the preview webview, and closes the temporary tab immediately.
