...
// index.ts
var exports_WebToken = {};
__export(exports_WebToken, {
  webToken: () => webToken
});
module.exports = __toCommonJS(exports_WebToken);
...
class webToken {
  ...
  encrypt(data) {
    try {
      const iv = import_crypto.randomBytes(webToken.IV_LENGTH);
      const cipher = import_crypto.createCipheriv(this.algorithm, this.hashedSecret(), iv);
  ...
  decrypt(encryptedData) {
    try {
      ...
      const decipher = import_crypto.createDecipheriv(this.algorithm, this.hashedSecret(), iv);
  ...
  createHMAC(data) {
    return import_crypto.createHash("sha256").update(data + this.secret).digest("hex");
  }
  constantTimeCompare(a, b) {
    ...
    return import_crypto.timingSafeEqual(bufferA, bufferB);
  }
  generateJTI() {
    return import_crypto.randomBytes(16).toString("hex");
  }
  ...
  hashedSecret() {
    return import_crypto.createHash("sha256").update(this.secret).digest().slice(0, 32);
  }
  static generateSecureSecret(length = 64) {
    return import_crypto.randomBytes(length).toString("hex");
  }
  getTokenInfo() {
    return {
